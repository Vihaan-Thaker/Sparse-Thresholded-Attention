#!/usr/bin/env python3
"""
shared_selection_triton_YaRN_exact.py

EXACT-MAX shared kernel with MARGIN-WEIGHTED ranking AND **YaRN** positioning
(the YaRN counterpart of shared_selection_triton_exact_2.py). Identical to that file EXCEPT it uses
YaRN instead of PI for positional encoding EVERYWHERE (the exact-max threshold scores in Kernel A's
two passes, and the repositioned attention in Kernel C):

  * positions are NOT compressed (pos_scale = 1.0);  * inv_freq = YaRN NTK-by-parts frequencies;
  * rope_scale = YaRN attention temperature mscale = 0.1*ln(s)+1.
Both are read from the model's rotary embedding, so the MODEL MUST BE LOADED WITH yarn rope_scaling
(see install_shared_pi_forward). The reposition step uses raw positions (newpos = max(raw_pos, prev+1));
YaRN's modified frequencies keep those long positions in-distribution.
...
def shared_pi_attention(q, k, v, tau, inv_freq, pos_scale=1.0, pct=30.0, sm_scale=None,
    ...
    _select_vote_kernel[gridA](
        qh, kh, tau, inv_freq, votes, row_counts, wvotes,
    ...
    idx, newpos_k, newpos_q, shared_size, kept_size, kpct, overflow, kpct_meas, overflow20 = _build_sets(
        votes, float(pos_scale), n_global, n_local, pct, tile, k_max, q_pos_base=q_pos_base,
...
def install_shared_pi_forward(model, pct=30.0, context_window=4096, n_global=32, n_local=32,
                              ...
                              apply_mscale=True):
    """Patch LlamaAttention.forward to route through the EXACT-max shared-YaRN Triton kernels.

    YaRN variant: reads the YaRN NTK-by-parts frequencies + attention temperature (mscale) straight
    from the model's rotary embedding, so THE MODEL MUST BE LOADED WITH yarn rope_scaling
    (rope_type="yarn", factor=N/W, original_max_position_embeddings=W). Then rotary.inv_freq is the
    YaRN inv_freq and rotary.attention_scaling is mscale. Positions are NOT compressed (pos_scale=1);
    the extension is entirely in inv_freq + mscale. (max_len is unused here — kept for signature parity.)"""
    import transformers.models.llama.modeling_llama as ml
    rotary = model.model.rotary_emb
    inv_freq = rotary.inv_freq.detach().float()          # YaRN NTK-by-parts freqs (yarn-loaded model)
    mscale = float(getattr(rotary, "attention_scaling", 1.0))       # YaRN temperature 0.1*ln(s)+1
    if mscale == 1.0:
        print("WARNING [YaRN kernel]: rotary.attention_scaling == 1.0 — the model does NOT look "
              "yarn-loaded (mscale should be >1). Load it with rope_scaling rope_type='yarn'.", flush=True)
    # apply_mscale: True -> use the YaRN attention temperature (rope_scale = mscale); False -> disable it
    # (rope_scale = 1.0) to A/B whether the temperature helps ppl. YaRN inv_freq + pos_scale=1 are
    # unchanged either way, so ONLY the mscale^2 logit scaling is toggled (Kernel A threshold + Kernel C).
    rope_scale = mscale if apply_mscale else 1.0
    print(f"YaRN kernel: apply_mscale={apply_mscale} -> rope_scale={rope_scale:.4f} "
          f"(model mscale={mscale:.4f})", flush=True)
    ...
    def spi_forward(self, hidden_states, position_embeddings=None, attention_mask=None,
        ...
        # ── YaRN: NO position compression. pos_scale is fixed to 1.0 (RAW positions); the context
        #    extension lives entirely in the YaRN inv_freq + mscale (read from the yarn-loaded model's
        #    rotary_emb at install). Frozen at prefill for decode consistency, exactly like the PI path. ──
        if past_len == 0:
            self.spi_pos_scale = 1.0
