#!/usr/bin/env python3
"""
shared_selection_triton_hybrid_exact.py

EXACT-MAX shared kernel with MARGIN-WEIGHTED ranking and a **HYBRID** positional geometry:
Kernel A SELECTS with YaRN, Kernel C REPOSITIONS + ATTENDS with PI. Two independent
(inv_freq, pos_scale, rope_scale) triples are threaded through instead of one:

  Kernel A  (selection / threshold scoring):   YaRN
      * pos_scale_A = 1.0 (raw positions);  * inv_freq_A = YaRN NTK-by-parts frequencies;
      * rope_scale_A = YaRN attention temperature mscale = 0.1*ln(s)+1  (toggled by apply_mscale).
      -> decides which keys pass threshold + the freq/margin/expmass vote ranking, in YaRN space.
  _build_sets + Kernel C  (reposition + attention):   PI
      * pos_scale_C = min(1, W/N)  (compress selected keys into the trained window W);
      * inv_freq_C = BASE RoPE (theta=10000);  * rope_scale_C = 1.0 (no temperature).
      -> reposition newpos = max(PI_pos, prev+1) genuinely binds; attend with base RoPE at newpos.

Rationale: tests whether the strong PI-Exact ppl came from Kernel C's PI compaction (in-distribution
positions for the base model) rather than the selection geometry. Model MUST be loaded WITH yarn
rope_scaling (Kernel A reads yarn inv_freq + mscale from rotary_emb); the base RoPE inv_freq for
Kernel C is computed from head_dim + rope_theta at install. NOTE: selection (A) and attention (C) are
scored in DIFFERENT geometries by design, so the "top-X%" keys are ranked under YaRN but attended under PI.

The shared top-X% key set can be RANKED by a weighted score:

  select_score = "freq"     top-X% by vote FREQUENCY  V(k)=sum_q keep(q,k)          (= the original kernel)
                 "margin"   top-X% by summed MARGIN    W(k)=sum_q keep(q,k)*(s-m_q+tau)   (delta in [0,tau])
                 "expmass"  top-X% by summed MASS       W(k)=sum_q keep(q,k)*exp(s-m_q)    (max-normalized wt)

Rationale: a key's max-normalized softmax weight for query q is exp(s(q,k)-m_q), monotone in the
threshold margin. Frequency voting ignores this (a key that is THE top token for one query ties with
one barely scraping past threshold). "expmass" ranks by the actual attention mass the tile places on a
key; "margin" is its cheaper linear proxy. The union size / budget (top-X% OF the union) and the
30->25->20 k_max cap are still driven by frequency; only the ranking metric changes.
  ...
  KERNEL A  _select_vote_kernel   (per head, per query-tile)  -- O(N), tensor-core dot, 2 passes
      ...
      pass 2: keep = score >= exact_max - tau ; votes(j) = sum over the tile's queries ;
              WMODE>0 also accumulates wvotes(j) = sum of margin / exp-mass over the tile -> HBM
  HOST      _build_sets()         rank by votes (freq) or wvotes (margin/expmass); top-X% cut
                                  (+global/local) -> gather index + newpos
...
if HAVE_TRITON:
    ...
    def _select_vote_kernel(
        Q, K, Tau, InvFreq, Votes, RowCounts, WVotes,
        ...
        for start_n in range(0, hi, BLOCK_N):
            ...
            # WMODE>0: also accumulate the weighted score per key over the tile's queries.
...
def _build_sets(votes, pos_scale, n_global, n_local, pct, T, k_max, q_pos_base=0, measure=False,
    ...
    select_score: "freq" ranks the top-X% by vote frequency (votes); "margin"/"expmass" rank by the
    per-key weighted score wvotes (H,nt,N). The UNION size and budget (top-X% of the union) are always
    taken from frequency (votes>0); only the ranking metric used for the cutoff changes."""
    H, nt, N = votes.shape
    dev = votes.device
    ...
    vf = votes.float()
    Usize = (vf > 0).sum(-1)                                            # (H,nt)  union size (frequency)
    # ranking metric: frequency, or the weighted score with frequency as an epsilon tiebreak so that
    # exact ties (e.g. many zero-margin keys) resolve deterministically and stay union-aware.
    if select_score == "freq" or wvotes is None:
        rank_val = vf
    else:
        rank_val = wvotes.float() + 1e-6 * vf
    ...
    # Order statistics only: `sv` is read solely at index (budget-1), and
    # budget = ceil(p% * Usize) with Usize <= N, so nothing above rank ceil(max_pct% * N) can ever be
    # touched. topk to exactly that width instead of sorting all N. The value at any rank < K_ord is
    # identical to the full sort's (sorted order is determined by the multiset, so ties do not
    # matter -- and the threshold is applied as `>= v_cut`, which only reads the value). The measure
    # loop uses p < max(pct_steps), so it is covered by the same bound.
    #   sort: values fp32 (H,nt,N) + indices int64 (H,nt,N)   = 7.25 GB at N=24576
    #   topk: values fp32 (H,nt,K) + indices int64 (H,nt,K)   = 2.18 GB at K=7373
    # K_ord is a STATIC bound (Usize <= N), so no device->host sync is needed to compute it.
    K_ord = max(1, min(N, int(math.ceil(max(pct_steps) / 100.0 * N))))
    sv = torch.topk(rank_val, K_ord, dim=-1, largest=True, sorted=True).values   # (H,nt,K_ord)
    ...
    for p in pct_steps:
        ...
        v_cut_p = torch.gather(sv, -1, (budget_p - 1).clamp(0, K_ord - 1).unsqueeze(-1))
        shared_p = rank_val >= v_cut_p                                  # (H,nt,N) tie-inclusive
    ...
    if measure:
        ...
        for p in range(floor_pct - 1, 0, -1):                          # 19,18,...,1
            ...
            v_cut_m = torch.gather(sv, -1, (budget_m - 1).clamp(0, K_ord - 1).unsqueeze(-1))
            kept_m = (rank_val >= v_cut_m) | glob_loc[None]             # (H,nt,N)
    ...
    # is O(N) instead of O(N log N) -- and it never materialises an (H,nt,N) int64 tensor. The old
    # sort needed three of them live at once (masked_pos, plus sort's values AND indices): ~14.5 GB
    # at N=24576, against ~0.8 GB for the buffer below. Measured on pq_exact @ 24k: 64.4 -> 49.9 GB
    # peak, with per-sequence ppl bit-identical.
    #
    # The `rank0 < k_max` guard is REQUIRED here (unlike pq_exact, which guarantees
    # kept_size <= k_max): tiles still over budget after the 30->25->20% steps stay unresolved, and
    # the old `sort(...)[:, :, :k_max]` silently truncated them by keeping the k_max SMALLEST
    # positions. Dropping ranks >= k_max reproduces that exactly -- rank r is by construction the
    # (r+1)-th smallest kept position -- and without it those keys would scatter out of bounds.
    # Non-kept and over-budget keys all land in the dump slot k_max, which is then sliced off; they
    # collide there, but every writer stores to a slot we discard, so that is harmless.
...
# Orchestration
# ──────────────────────────────────────────────────────────────────────────────
def shared_pi_attention(q, k, v, tau,
                        inv_freq_a, pos_scale_a, rope_scale_a,     # Kernel A  (YaRN selection)
                        inv_freq_c, pos_scale_c, rope_scale_c,     # _build_sets + Kernel C (PI attend)
                        pct=30.0, sm_scale=None, n_global=32, n_local=32, tile=32, k_max=1024,
                        block_n=64, block_k=64, q_pos_base=0, measure=False, select_score="freq"):
    """q,k,v: (1,H,N,D) RAW. q_pos_base = absolute position of the first query (0 at prefill).
    HYBRID geometry: Kernel A scores/selects with (inv_freq_a, pos_scale_a, rope_scale_a) = YaRN;
    _build_sets reposition + Kernel C attend with (inv_freq_c, pos_scale_c, rope_scale_c) = PI.
    measure=True runs the below-20% instrumentation (host syncs); leave False for speed.
    select_score: "freq" (frequency voting) | "margin" | "expmass" (weighted key ranking).
    ...
    inv_freq_a = inv_freq_a.float().contiguous()      # YaRN NTK-by-parts freqs (Kernel A)
    inv_freq_c = inv_freq_c.float().contiguous()      # base RoPE freqs        (Kernel C)
    ...
    # weighted-score buffer; dummy (1x1) when freq so the kernel skips the accumulation entirely
    wvotes = torch.zeros((H, nt, N) if WMODE > 0 else (H, 1, 1),
                         dtype=torch.float32, device=q.device)
    gridA = (nt, H)
    _select_vote_kernel[gridA](
        qh, kh, tau, inv_freq_a, votes, row_counts, wvotes,
    ...
    idx, newpos_k, newpos_q, shared_size, kept_size, kpct, overflow, kpct_meas, overflow20 = _build_sets(
        votes, float(pos_scale_c), n_global, n_local, pct, tile, k_max, q_pos_base=q_pos_base,
        measure=measure, wvotes=wvotes, select_score=select_score)

    out = torch.empty((H, N, D), dtype=q.dtype, device=q.device)
    gridC = (nt, H)
    _gather_attn_kernel[gridC](
        qh, kh, vh, inv_freq_c, idx, newpos_k, newpos_q, out,
        sm_scale, float(rope_scale_c), int(q_pos_base),
        qh.stride(0), qh.stride(1), qh.stride(2),
        kh.stride(0), kh.stride(1), kh.stride(2),
...
@torch.no_grad()
def _decode_build_sets(q, K, tau, inv_freq_a, q_pos, pos_scale_a, rope_scale_a, sm_scale,
                       n_global, n_local, k_max, pct_decode, pos_scale_c, scores=None):
    """Per-query SELECTION (YaRN geometry) + reposition (PI geometry), VECTORIZED over heads.
    ...
    Rule: SELECT via YaRN scores (inv_freq_a, pos_scale_a, rope_scale_a): threshold keep (score>=max-tau)
    + global + local + self, optional top-pct_decode% of passers (by score), then k_max cap by score;
    then REPOSITION with PI (newpos = rank + cummax(cpos_c - rank), cpos_c = idx*pos_scale_c).
    scores: precomputed per-key YaRN scores (H,M); if None they are computed here in torch."""
    H, M, D = K.shape
    dev = K.device
    ...
    if scores is None:
        ...
        cpos_sel = pos.float() * pos_scale_a
        qs = _rope_rotate(q, torch.tensor(float(q_pos) * pos_scale_a, device=dev), inv_freq_a, rope_scale_a)  # (H,D)
        Ks = _rope_rotate(K, cpos_sel, inv_freq_a, rope_scale_a)                                              # (H,M,D)
    ...
    cpos = kj.float() * pos_scale_c                                       # PI compression (Kernel C geometry)
...
def install_shared_pi_forward(model, pct=30.0, context_window=4096, n_global=32, n_local=32,
                              ...
                              prefill_mode="shared", measure=False, select_score="freq",
                              apply_mscale=True):
    """Patch LlamaAttention.forward to route through the HYBRID shared Triton kernels.

    HYBRID geometry: Kernel A SELECTS with YaRN, Kernel C REPOSITIONS + ATTENDS with PI.
      * Kernel A (YaRN): reads the YaRN NTK-by-parts inv_freq + attention temperature (mscale) from the
        model's rotary embedding, so THE MODEL MUST BE LOADED WITH yarn rope_scaling
        (rope_type="yarn", factor=N/W, original_max_position_embeddings=W). pos_scale_A = 1.0 (raw).
      * Kernel C (PI): uses BASE RoPE inv_freq (theta from config, computed here since rotary.inv_freq is
        now the YaRN one), pos_scale_C = min(1, W/N) frozen at prefill, rope_scale_C = 1.0.
    apply_mscale toggles the YaRN temperature in Kernel A ONLY (Kernel C is PI, always rope_scale=1)."""
    import transformers.models.llama.modeling_llama as ml
    rotary = model.model.rotary_emb
    inv_freq_a = rotary.inv_freq.detach().float()        # YaRN NTK-by-parts freqs (Kernel A / selection)
    mscale = float(getattr(rotary, "attention_scaling", 1.0))       # YaRN temperature 0.1*ln(s)+1
    if mscale == 1.0:
        print("WARNING [hybrid kernel]: rotary.attention_scaling == 1.0 — the model does NOT look "
              "yarn-loaded (mscale should be >1). Load it with rope_scaling rope_type='yarn'.", flush=True)
    rope_scale_a = mscale if apply_mscale else 1.0       # YaRN temperature in Kernel A (selection) only

    # ── Kernel C base RoPE inv_freq: rotary.inv_freq is YaRN now, so recompute the ORIGINAL base freqs
    #    (theta^{-2i/D}) from head_dim + rope_theta. This is exactly the inv_freq a plain-loaded model
    #    would expose, i.e. what the pure PI exact kernel uses. ──
    cfg = model.config
    ...
    inv_freq_c = 1.0 / (theta ** (torch.arange(0, head_dim, 2, dtype=torch.float32) / head_dim))
    assert inv_freq_c.numel() == inv_freq_a.numel(), \
        f"base/yarn inv_freq length mismatch: {inv_freq_c.numel()} vs {inv_freq_a.numel()}"
    print(f"hybrid kernel: A=YaRN(mscale={mscale:.4f}, apply_mscale={apply_mscale} -> "
          f"rope_scale_A={rope_scale_a:.4f}), C=PI(base RoPE theta={theta:.0f}, "
          f"pos_scale_C=min(1,W/N))", flush=True)

    for li, layer in enumerate(model.model.layers):
        sa = layer.self_attn
        dev = sa.q_proj.weight.device
        sa.spi_inv_freq_a = inv_freq_a.to(dev)            # YaRN freqs (Kernel A selection)
        sa.spi_rope_scale_a = rope_scale_a               # YaRN mscale (or 1 if apply_mscale=False)
        sa.spi_inv_freq_c = inv_freq_c.to(dev)           # base RoPE freqs (Kernel C attention)
        sa.spi_rope_scale_c = 1.0                        # PI has no attention temperature
        sa.spi_pos_scale_c = None                         # PI compression, set once at prefill
        ...
        sa.spi_max_len = max_len                          # target length for frozen pos_scale_C
        ...
        sa.spi_select_score = select_score                # "freq" | "margin" | "expmass" (shared ranking metric)
    ...
    def spi_forward(self, hidden_states, position_embeddings=None, attention_mask=None,
        ...
        # ── HYBRID: Kernel A uses raw positions (pos_scale_A = 1.0); _build_sets + Kernel C use PI
        #    compression pos_scale_C = min(1, W/N), frozen at prefill (from spi_max_len or N) for decode
        #    consistency, exactly like the pure PI kernel. ──
        if past_len == 0:
            L_target = self.spi_max_len if self.spi_max_len is not None else N
            self.spi_pos_scale_c = min(1.0, self.spi_W / float(L_target))
        pos_scale_c = self.spi_pos_scale_c
        ...
        if past_len == 0 and getattr(self, "spi_prefill_mode", "shared") == "shared":
            ...
            out, shared_size, kept_size, kpct, overflow, kpct_meas, overflow20 = shared_pi_attention(
                q, k, v, self.tau_head_vec,
                self.spi_inv_freq_a, 1.0, self.spi_rope_scale_a,           # Kernel A (YaRN select)
                self.spi_inv_freq_c, pos_scale_c, self.spi_rope_scale_c,   # _build_sets + Kernel C (PI)
                pct=self.spi_pct, sm_scale=self.scaling, n_global=self.spi_ng,
                ...
                select_score=getattr(self, "spi_select_score", "freq"))
            ...
            for i in range(N):
                ...
                if select_backend == "triton":
                    # SELECT scores in YaRN geometry (raw pos, yarn freqs, mscale)
                    scores = _decode_scores_kernel(qi, Ki, self.spi_inv_freq_a, P, 1.0,
                                                   self.spi_rope_scale_a, self.scaling, block_n=self.spi_block_n)
                idx, npk, npq = _decode_build_sets(
                    qi, Ki, self.tau_head_vec, self.spi_inv_freq_a, P, 1.0,     # select: YaRN
                    self.spi_rope_scale_a, self.scaling, self.spi_ng, self.spi_nl, self.spi_kmax,
                    getattr(self, "spi_pct_decode", 100.0), pos_scale_c, scores=scores)  # reposition: PI
                if attend_backend == "triton":
                    oh = _decode_attend_kernelC(qi, Ki, Vi, idx, npk, npq, self.spi_inv_freq_c,  # attend: PI
                                                self.spi_rope_scale_c, self.scaling, self.spi_kmax,
                                                q_pos=P, block_k=self.spi_block_k)
                else:
                    oh = _decode_attend_torch(qi, Ki, Vi, idx, npk, npq, self.spi_inv_freq_c,    # attend: PI
                                              self.spi_rope_scale_c, self.scaling)
                outs.append(oh)
